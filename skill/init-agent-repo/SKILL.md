...
description: Transform any repository into an agent-native structure. Generates .agent/MANIFEST.yaml, .agent/INVARIANTS.md, .agent/IMPACT_MAP.yaml, and AGENT.md — the metadata layer proven to improve AI agent task success rates by 25+ percentage points. Use when starting a new project or upgrading an existing repo for AI agent workflows.
---
...
Research across 40 experiment runs showed that agent-native repos achieve **+25pp higher task success rates** vs traditional repos. The mechanism: agents in traditional repos "prematurely commit" (start editing after reading only 4–7 files). Agent-native metadata forces upfront context-building, converting unknown unknowns into known unknowns before any edits are made.

The four files you will generate are the core of this system.
...
Generate all four files. **Do not skip any.** Create the `.agent/` directory if it doesn't exist.
...
2. Verify `.agent/` directory contains exactly: `MANIFEST.yaml`, `INVARIANTS.md`, `IMPACT_MAP.yaml`
